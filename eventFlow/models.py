...
class User(UserMixin, db.Model):
    ...
    def get_upcoming_tickets(self):
        ...
        free_tickets = (
            ...
            .filter(EventRegistration.user_id == self.id, Event.date > current_time)
            .all()
        )

        # Get paid tickets
        paid_tickets = (
            Order.query.join(Event)
            .filter(
...
class Event(db.Model):
    ...
    ticket_types = db.relationship(
        "TicketType", back_populates="event", lazy=True, cascade="all, delete-orphan"
    )
    ...
    def is_user_registered(self, user):
        return (
            EventRegistration.query.filter_by(event_id=self.id, user_id=user.id).first()
            is not None
        )
    ...
    def get_sales_stats(self):
        ...
        total_tickets_sold = sum(
            order.get_ticket_count() for order in self.orders if order.is_paid
        )
...
class Order(db.Model):
    ...
    def create_from_checkout(
        user_id, event_id, total_amount, payment_id, ticket_details
    ):
